...
const getSubmissionsForChallenge = asyncHandler(async (req, res) => {
  const submissions = await SubmissionsController.getUserSubmissionsForChallenge(
    req.user,
...
const getAllSubmissions = asyncHandler(async (req, res) => {
  const submissions = await SubmissionsController.getAllSubmissionsForEvent(
    req.user.event
  );
  return res.status(200).json(submissions);
});
...
router
  .route("/")
  .get(passport.authenticate("token", { session: false }), getSubmissions);

router
  .route("/all")
  .get(passport.authenticate("admin", { session: false }), getAllSubmissions);
...
router
  ...
  .post(
    passport.authenticate("token", { session: false }),
    createSubmissionForChallenge
  );

router
  .route("/review/:submissionId")
  .post(passport.authenticate("admin", { session: false }), reviewSubmission);
