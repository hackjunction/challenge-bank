...
controller.getActiveSubmissionsForChallenge = (user, challengeId) => {
  return controller
    .getSubmissionsForChallenge(user, challengeId)
    .then(submissions => {
...
controller.createSubmission = async (user, challengeId, answer) => {
  const activeSubmissions = await controller.getActiveSubmissionsForChallenge(
    user,
    challengeId
  );
  ...
  if (activeSubmissions.length > 0) {
    const acceptedSubmission = _.find(
      activeSubmissions,
      s => s.reviewStatus === 2
    );
    ...
    const pendingSubmission = _.find(
      pendingSubmission,
      s => s.reviewStatus === 2
    );
    if (pendingSubmission) {
      throw new Error(
        "You already have a submission pending review for this challenge"
      );
    }
  }
