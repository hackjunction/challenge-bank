...
function getScoresForEvents(req, res) {
  return mongoose
    ...
    .then(submissions => {
      ...
      const maxParticipants = _.maxBy(submissions, "event.participantCount")
      ...
      _.forOwn(groupedByEvent, (submissions, eventId) => {
        ...
        eventArray.push({
          ...
          submissionCount,
          weightedPoints: getWeightedScore(
            points,
            event.participantCount,
            maxParticipants
          ),
...
function getPointsForSubmission(submission) {
  ...
  const p = PointsMap[submission.challengeDifficulty]
    ? PointsMap[submission.challengeDifficulty]
    : 0;
