        ...
        UsersService.register(username.value, password.value, secretCode.value)
            ...
            .catch(err => {
                if (err.response) {
                    ...
                    notification.error({
                        message: 'Signup failed',
                        description: "Something wen't wrong... Are you connected to the internet?"
                    });
                }
            })
            .finally(() => {
    ...
    return (
        <React.Fragment>
            <InputWrapper label="Username" error={username.error}>
                <Input {...username} type="text" placeholder="Your awesome username" size="large" />
            </InputWrapper>
            <InputWrapper label="Password" error={password.error}>
                <Input {...password} type="password" placeholder="Something super secret" size="large" />
            </InputWrapper>
